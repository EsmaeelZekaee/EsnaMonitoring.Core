    ...
    public static class Extensions
    {
        public static Stream GetEmbeddedResource(this Assembly assembly, string resourceName)
        {
            ...
            var embeddedResourceName =
                assembly.GetManifestResourceNames().FirstOrDefault(_ => _.EndsWith(resourceName));

            if (embeddedResourceName == null)
                throw new ArgumentException(
                    $"A resource with name {resourceName} could not be found",
                    nameof(resourceName));

            return assembly.GetManifestResourceStream(embeddedResourceName)
                   ?? throw new ArgumentException($"Unable to read manifest resource stream for {resourceName}");
        }
        ...
        public static string Truncate(this string str, int length)
        {
            ...
            if (length < 1) throw new ArgumentException("Cannot be a negative number", nameof(length));
