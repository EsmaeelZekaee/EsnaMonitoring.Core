namespace ModbusUtility
{
    ...
    internal class Modbus
    {
        ...
        public Result DetectDevice(byte unitId, out string name)
        {
            ...
            {
                if (array[0] != array2[0] || array[1] != array2[1]) result = Result.RESPONSE;
                else if (quantity != array2[2]) result = Result.BYTECOUNT;
                else
        ...
        public Result MaskWriteRegister(byte unitId, ushort address, ushort ANDMask, ushort ORMask)
        {
            ...
            var result = this.TxRx.TxRx(array, 8, array2, 8);
        ...
        public Result ReadFlags(byte unitId, byte function, ushort address, ushort quantity, bool[] Bools, int offset)
        {
            ushort num = 0;
            ushort num2 = 0;
            if (function < 1 || function > 127) return Result.FUNCTION;
            if (quantity < 1 || quantity > 2000) return Result.QUANTITY;
            if (quantity + offset > Bools.GetLength(0)) return Result.QUANTITY;
            var array = new byte[8];
            var array2 = new byte[261];
            array[0] = unitId;
            array[1] = function;
            ...
            array[4] = ByteAccess.HiByte(quantity);
            array[5] = ByteAccess.LoByte(quantity);
            var num3 = (quantity + 7) / 8 + 3;
            var result = this.TxRx.TxRx(array, 6, array2, num3);
            if (result == Result.SUCCESS)
            {
        ...
        {
            if (function < 1 || function > 127) return Result.FUNCTION;
            if (quantity < 1 || quantity > 125) return Result.QUANTITY;
            if (quantity + offset > registers.GetLength(0)) return Result.QUANTITY;
            var array = new byte[8];
            var array2 = new byte[261];
            array[0] = unitId;
            array[1] = (byte)(function & 255);
            ...
            array[4] = ByteAccess.HiByte(quantity);
            array[5] = ByteAccess.LoByte(quantity);
            var responseLength = 3 + quantity * 2;
            var result = this.TxRx.TxRx(array, 6, array2, responseLength);
            if (result == Result.SUCCESS)
            {
                if (array[0] != array2[0] || array[1] != array2[1]) result = Result.RESPONSE;
                else if (quantity * 2 != array2[2]) result = Result.BYTECOUNT;
                else
                    for (var i = 0; i < quantity; i++)
                        registers[i + offset] =
                            (short)((array2[2 * i + 4] & byte.MaxValue) | ((array2[2 * i + 3] & byte.MaxValue) << 8));
            }
        ...
        {
            if (readSize < 1 || readSize > 123) return Result.QUANTITY;
            if (writeSize < 1 || writeSize > 121) return Result.QUANTITY;
            if (readSize > readRegisters.GetLength(0)) return Result.QUANTITY;
            if (writeSize > writeRegisters.GetLength(0)) return Result.QUANTITY;
        ...
        public Result ReportSlaveID(byte unitId, out byte byteCount, byte[] deviceSpecific)
        {
            ...
            var array2 = new byte[255];
            array[0] = unitId;
            array[1] = 17;
            byteCount = 0;
            var result = this.TxRx.TxRx(array, 2, array2, int.MaxValue);
            if (result == Result.SUCCESS)
            {
                if (array[0] != array2[0])
                {
                ...
                {
                    byteCount = array2[2];
                    var num = Math.Min(array2[2], deviceSpecific.GetLength(0));
                    for (var i = 0; i < num; i++) deviceSpecific[i] = array2[i + 3];
                }
            }
        ...
        {
            if (function < 1 || function > 127) return Result.FUNCTION;
            if (quantity < 1 || quantity > 1968) return Result.QUANTITY;
            if (quantity + offset > Bools.GetLength(0)) return Result.QUANTITY;
            ...
            for (var i = 0; i < quantity; i++)
            {
                if (Bools[i + offset]) b2 |= (byte)(1 << num);
                num += 1;
                if (num == 8)
                {
                    num = 0;
        ...
        {
            if (function < 1 || function > 127) return Result.FUNCTION;
            if (quantity < 1 || quantity > 123) return Result.QUANTITY;
            if (quantity + offset > registers.GetLength(0)) return Result.QUANTITY;
            var array = new byte[265];
            var array2 = new byte[8];
            array[0] = unitId;
        ...
        public Result WriteSingleCoil(byte unitId, ushort address, bool coil)
        {
            ...
            array[5] = 0;
            var result = this.TxRx.TxRx(array, 6, array2, 6);
            if (result == Result.SUCCESS && array[0] != 0)
                for (var i = 0; i < 6; i++)
                    if (array[i] != array2[i])
                        result = Result.RESPONSE;
